...
void ecriture(char* s) {

    FILE* fichier = fopen("sortie_standard.txt", "a");
    fprintf(fichier, "%s", s);
    fclose(fichier);
...
as of 0.4.0 all options are specified in config file, see in '/home/username/.config/cava/' \n";
    ...
    //variables perso rajoutées
    double max = 0;
    double temporaire = 0;
    int max_idx = 0;

    //effacer le fichier sortie standard
    FILE* file = fopen("sortie_standard.txt", "w");
    fclose(file);
...
#ifdef PULSE
        if (im == 3) {
            ecriture("on est en input pulse\n");
            ...
            while (!resizeTerminal) {
                ...
                if (sleep < framerate * 5) {
                    ...
                    for (int i = 0; i < (M / 2 + 1); i++) {
                        ...
                        temporaire = pow(pow(*outl[i][0], 2) + pow(*outl[i][1], 2), 0.5);
                    ...
                    if (max > 2000000) {
                        ...
                        sprintf(s, "Maximum de décibel: %lf. A l'échantillon numéro: %i.\n", max, max_idx);
                        ecriture(s);
                        free(s);
                    }
